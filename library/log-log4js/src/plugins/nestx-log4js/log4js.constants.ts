...
export const LOG4JS_OPTIONS: symbol = Symbol('NEST_LOG4JS_OPTIONS')
export const LOG4JS_LOGGER: symbol = Symbol('NEST_LOG4JS_LOGGER')

export const LOG4JS_DEFAULT_LAYOUT: PatternLayout = {
  type: 'pattern',
...
export const LOG4JS_NO_COLOUR_DEFAULT_LAYOUT: PatternLayout = {
  type: 'pattern',
  // log4js default pattern %d{yyyy-MM-dd HH:mm:ss:SSS} [%thread] %-5level %logger{36} - %msg%n
  // we use process id instead thread id
  pattern: '%d{yyyy-MM-dd hh:mm:ss:SSS} %-5.5p --- [%15.15x{name}] %40.40f{3}  : %m',

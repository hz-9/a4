 ...
 * @LastEditTime : 2024-06-29 18:30:24
 */
...
describe('plugin - Address', () => {
  ...
  it('Get default nterface name', async () => {
    const name1 = await Address.getDefaultInterfaceName(NetworkFamily.IPv4)
    const name2 = await Address.getDefaultInterfaceName(NetworkFamily.IPv6)

    const name3 = await Address.getDefaultInterfaceName(NetworkFamily.IPv4, 'Hello World A4 Network')
    const name4 = await Address.getDefaultInterfaceName(NetworkFamily.IPv6, 'Hello World A4 Network')

    expect(name1).toBeTruthy()
    expect(name2).toBeTruthy()
    expect(name3).toBeUndefined()
    expect(name4).toBeUndefined()
  })
})
